...
def _get_pos_list_from(tree, node, poslst, index, coords, gap):
    ...
    if node and node.key == tree.Root.key:
        ...
        positions = _get_pos_list_from(tree, tree.Root.right, positions, 1 + tree.get_element_count(node.left), (0, 0),
                                       gap)
        return positions
    elif node:
        ...
        positions = _get_pos_list_from(tree, node.right, positions, 1 + index + tree.get_element_count(node.left),
                                       new_coords, gap / 2)
        return positions
    else:
        return positions
